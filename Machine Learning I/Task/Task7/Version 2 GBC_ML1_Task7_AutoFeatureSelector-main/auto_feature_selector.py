...
def autoFeatureSelector(dataset_path, methods=[]):
    ...
    if 'mutual-info' in methods:
        mi_support, mi_feature = mutual_info_selector(X, y, num_feats)
        support_dict['mutual-info'] = mi_support
        feature_dict['mutual-info'] = mi_feature
    if 'lasso' in methods:
        lasso_support, lasso_feature = lasso_selector(X, y, num_feats)
        support_dict['lasso'] = lasso_support
        feature_dict['lasso'] = lasso_feature
    if 'xgb' in methods:
        xgb_support, xgb_feature = xgb_selector(X, y, num_feats)
        support_dict['xgb'] = xgb_support
        feature_dict['xgb'] = xgb_feature
    if 'pca' in methods:
        pca_model, pca_features = pca_selector(X, num_feats)
        feature_dict['pca'] = pca_features  # PCA returns components, not original features
        # PCA does not produce support, so we skip adding to support_dict
